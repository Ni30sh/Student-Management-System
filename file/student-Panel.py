    ...
    def submit_details():
        ...
        if existing_data:
            ...
            cur.execute("INSERT INTO INFORMATION VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",
                    (account_val, name_val, aadhar_val, dob_val, address_val, contact_val, email_val, gender_val, date))
            conn.commit()
            ...
            box = messagebox.askyesnocancel("Notification", "ADDED SUCCESSFULLY... WANT TO CLEAR THE FORM")
            if box == True:
                ID.set("")
                NAME.set("")
                AADHAR.set("")
...
def principal_access():
    ...
    def open_principal_portal():
        ...
        show_details_button = Button(principal_window, text="Details",activeforeground="black",activebackground="white",
...
def exit_studt():
    ...
    if rt == True:
        qwe.destroy()
# __________________________________________________________________________________________connection of database
def database_window():
    ...
    password = Label(data, text="Password : ",  bg="white", font=("times", 13, "bold"), anchor="s")
